...
const TCHAR *g_szAppName = _T("网游更新系统");
const BOOL g_uploader = TRUE;

WCHAR g_workDir[MAX_PATH];
HINSTANCE g_hInstance;
HWND g_hWndMain, g_hWndTmpl, g_hWndFocus;
HWND g_hWndTree;
HWND g_hWndDownloading, g_hWndNet;
HWND g_hWndLog;
...
HTREEITEM g_htiWorking, g_htiNet, g_htiLocal;
...
static int search_cmp(const void *p1, const void *p2)
{
                       ...
                       int nCmdShow)
{
    ...
    ptrArray_init(&g_searchRes, idxNet_cmpId);
...
_main_exit:
    ...
    ptrArray_free(&g_searchRes, NULL);
...
BOOL SaveOptions()
{
    ...
    ini_writeStr(hFile, "ServerPwd", g_serverPwd);
...
BOOL ReadOptions()
{
    ...
    while (1)
    {
        ...
        else if (0==_stricmp(pLine, "ServerPwd") && strlen(pEq) < 64)
            strcpy_s(g_serverPwd, 64, pEq);
    }
    free(fileData);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    if (rcScr.right >= 1200) cx = 1200; else cx = rcScr.right;
    if (rcScr.bottom >= 708) cy = 708; else cy = rcScr.bottom - 40;

    g_hWndMain = CreateWindow(
        _T("gmUIWndClass"), g_szAppName,
        WS_OVERLAPPEDWINDOW,
        rcScr.right - cx, rcScr.bottom - cy, cx, cy,
...
static void Tree_RemoveCategoryItems()
{
    ...
    while (1)
    {
        hti = TreeView_GetChild(g_hWndTree, g_htiNet);
        if (!hti) break;
        TreeView_DeleteItem(g_hWndTree, hti);
    }

    while (1)
    {
        hti = TreeView_GetChild(g_hWndTree, g_htiLocal);
        if (!hti) break;
        TreeView_DeleteItem(g_hWndTree, hti);
    }
}
...
static void Tree_AddCategoryItems()
{
    ...
    {
        hti = TVI_FIRST;
        hti = _AddItemToTree(g_hWndTree, g_htiNet, hti,
            UnicodeToTSTR((WCHAR *)ptrArray_nth(&g_categories, i), szText, 256),
            PAGE_NET_IDX*1000+1+i);
    }
    TreeView_Expand(g_hWndTree, g_htiNet, TVE_EXPAND);

    for (i=0; i<ptrArray_size(&g_categories); i++)
    {
        hti = TVI_FIRST;
        hti = _AddItemToTree(g_hWndTree, g_htiLocal, hti,
            UnicodeToTSTR((WCHAR *)ptrArray_nth(&g_categories, i), szText, 256),
            PAGE_LOCAL_IDX*1000+1+i);
    }
    TreeView_Expand(g_hWndTree, g_htiLocal, TVE_EXPAND);
}
...
void Tree_CreateWindow(HWND hwndParent)
{
    ...
    g_htiWorking = _AddItemToTree(g_hWndTree, TVI_ROOT, TVI_FIRST, _T("正在下载"), PAGE_DOWNLOADING*1000);
    g_htiNet = _AddItemToTree(g_hWndTree, TVI_ROOT, g_htiWorking, _T("所有资源"), PAGE_NET_IDX*1000);
    g_htiLocal = _AddItemToTree(g_hWndTree, TVI_ROOT, g_htiNet, _T("已下载"), PAGE_LOCAL_IDX*1000);
}

static void Tree_OnNotify(NMHDR *lpNmhdr)
{
    NMTREEVIEW *pnmtv;
    int newPage, newSubPage;
    int oldButton, newButton;
    ...
    case TVN_SELCHANGED:
        ...
        newSubPage = newPage % 1000;
        newPage /= 1000;
        if (newPage >= PAGE_DOWNLOADING && newPage <= PAGE_LOCAL_IDX)
        {
            if (newPage != g_currPage)
            {
                switch (g_currPage)
                {
                case PAGE_DOWNLOADING: oldButton = IDM_WORKING; break;
                case PAGE_NET_IDX: oldButton = IDM_NET; break;
                case PAGE_LOCAL_IDX: oldButton = IDM_LOCAL; break;
                default: oldButton = -1; break;
                }
                switch (newPage)
                {
                case PAGE_DOWNLOADING: newButton = IDM_WORKING; break;
                case PAGE_NET_IDX: newButton = IDM_NET; break;
                case PAGE_LOCAL_IDX: newButton = IDM_LOCAL; break;
                default: newButton = -1; break;
                }
                if (oldButton > 0)
                    SendMessage(g_hWndToolBar, TB_SETSTATE, oldButton, MAKELONG(TBSTATE_ENABLED, 0));
                if (newButton > 0)
                    SendMessage(g_hWndToolBar, TB_SETSTATE, newButton, MAKELONG(TBSTATE_CHECKED|TBSTATE_ENABLED, 0));
            }
            if (newPage != g_currPage || newSubPage != g_currSubPage)
                PostMessage(g_hWndMain, WM_COMMAND, IDM_CHANGE_PAGE, MAKELONG(newPage, newSubPage));
        }
        break;
    }
}
...
static void Downloading_CreateWindow(HWND hwndParent)
{
    ...
    SetListViewColumn(g_hWndDownloading, 1, _T("名称"), 230, LVCFMT_LEFT);
...
static TCHAR *GetActionString(struct idx_downloading *dn, TCHAR szAction[256])
{
    ...
    if (!dn->action) _tcscpy_s(szAction, 256, _T("排队"));
    else if (dn->action & TS_UPLOADING) _tcscpy_s(szAction, 256, _T("上传"));
    else if (dn->action & TS_SEEDING)
    {
        ...
        else _tcscpy_s(szAction, 256, _T("供种"));
    }
    else
    {
        szAction[0] = 0;
        if (dn->action & TS_CHECKING) _tcscpy_s(szAction, 256, _T("检查"));
        else if (dn->action & TS_CONTINUING) _tcscpy_s(szAction, 256, _T("恢复"));
        else if (dn->action & TS_PREPARING) _tcscpy_s(szAction, 256, _T("准备"));
        else if (dn->action & TS_TRANSFERING) _tcscpy_s(szAction, 256, _T("转储"));
        else if (dn->action & (TS_DOWNLOADING|TS_UPDATING))
        {
            ...
            _tcscpy_s(szAction, 256, _T("下载"));
        }

        if (dn->action & TS_ERROR) _tcscat_s(szAction, 256, _T(" | 出错"));
        if (dn->action & TS_PAUSED) _tcscat_s(szAction, 256, _T(" | 暂停"));
    }
    return szAction;
}
...
static void OnDownloadingAdded(CHAR *id)
{
    ...
    dnNew = (struct idx_downloading *)malloc(sizeof(struct idx_downloading)); if (!dnNew) return;
...
static void Downloading_OnNotify(NMHDR *lpNmhdr)
{
    ...
    case NM_RCLICK:
        ...
        if (!g_uploader)
        {
            cnt = GetMenuItemCount(hMenuTrackPopup);
            DeleteMenu(hMenuTrackPopup, cnt-1, MF_BYPOSITION);
            DeleteMenu(hMenuTrackPopup, cnt-2, MF_BYPOSITION);
        }
...
static void OnTrackerInfo(CHAR *pId)
{
    ...
    pti = (struct tracker_info *)ptrArray_findSorted(&g_trackerInfo, &ti);
    if (pti) *pti = ti;
    else
    {
        pti = (struct tracker_info *)malloc(sizeof(struct tracker_info));
        *pti = ti;
...
static void OnWaitingAdded(CHAR *id)
{
    ...
    dn = (struct idx_downloading *)malloc(sizeof(struct idx_downloading)); if (!dn) return;
...
static void OnUploadingAdded(CHAR *id)
{
    ...
    {
        dnOld = (struct idx_downloading *)li->data;
...
static void OnSeedingDeleted(CHAR *pId)
{
    ...
    strcpy_s(dnf.id, MAX_ID_LEN, pId);
    dn = (struct idx_downloading *)ptrArray_removeSorted(&g_seeding, &dnf);
    if (dn)
    {
        free(dn);
        g_seedingCnt --;
...
{
    TCHAR tszText[256], tszId[MAX_ID_LEN];

    _stprintf_s(tszText, 256, _T("%s 供种时间已到"), MbcsToTSTR(pId, tszId, MAX_ID_LEN));
    Log_AddLine(tszText);
}
...
void NetIdx_CreateWindow(HWND hwndParent)
{
    ...
    SetListViewColumn(g_hWndNet, 1, _T("名称"), 230, LVCFMT_LEFT);
...
static void _NetIdx_SetItemTextL(int iItem, struct idx_local *idxl)
{
    struct idx_net *idxn, idxnf = { 0 };
    TCHAR tszText[512];
    ...
    ListView_SetItemText(g_hWndNet, iItem, 1, UnicodeToTSTR(idxl->name, tszText, 512));
    ListView_SetItemText(g_hWndNet, iItem, 2, GetSizeString(idxl->size, tszText));
    ListView_SetItemText(g_hWndNet, iItem, 3, GetTimeString(idxn->lastUpdateTime, tszText));
    ListView_SetItemText(g_hWndNet, iItem, 4, UnicodeToTSTR(idxl->dir, tszText, 512));
    ListView_SetItemText(g_hWndNet, iItem, 5, GetTimeString(idxl->completeTime, tszText));
    ListView_SetItemText(g_hWndNet, iItem, 6,
        ptrArray_findSorted(&g_autoUpdate, idxl->id)?_T("自动"):_T(""));
}
...
void NetIdx_FillListView()
{
    ...
    {
    case PAGE_SEARCH:
        for (i=j=0; i<ptrArray_size(&g_searchRes); i++, j++)
        {
            idxn = (struct idx_net *)ptrArray_nth(&g_searchRes, i);
            lvI.iItem = i;
            lvI.iImage = ICON_NOTHING;
            lvI.pszText = MbcsToTSTR(idxn->id, tszText, 512);
            if (i < itemCount) ListView_SetItem(g_hWndNet, &lvI);
            else ListView_InsertItem(g_hWndNet, &lvI);

            _NetIdx_SetItemText(i, idxn);
        }
        break;
    ...
    case PAGE_NET_IDX:
        ...
        {
            idxn = (struct idx_net *)ptrArray_nth(&g_netIdx, i);
    ...
    case PAGE_LOCAL_IDX:
        for (i=j=0; i<ptrArray_size(&g_localIdx); i++)
        {
            idxl = (struct idx_local *)ptrArray_nth(&g_localIdx, i);
            if (!g_currSubPage || !wcscmp(idxl->category, g_currCategory) ||
                (!idxl->category[0] && !wcscmp(g_currCategory, L"~未分类")))
            {
                lvI.iItem = j;
                lvI.iImage = ICON_NOTHING;
                lvI.pszText = MbcsToTSTR(idxl->id, tszText, 512);
                if (j < itemCount) ListView_SetItem(g_hWndNet, &lvI);
                else ListView_InsertItem(g_hWndNet, &lvI);

                _NetIdx_SetItemTextL(j++, idxl);
            }
        }
        break;
    }
...
static void OnNetIdxAdded(CHAR *id)
{
    ...
    idxn = (struct idx_net *)malloc(sizeof(struct idx_net)); if (!idxn) return;
    memset(idxn, 0, sizeof(struct idx_net));
    if (!IdxNetFromUtf8String(idxn, id)) { free(idxn); return; }

    idxnOld = (struct idx_net *)ptrArray_findSorted(&g_netIdx, idxn);
    if (idxnOld)
    {
        *idxnOld = *idxn;
        free(idxn);
    ...
    if (g_currPage==PAGE_NET_IDX &&
        (!g_currSubPage || !wcscmp(idxn->category, g_currCategory) ||
        (!idxn->category[0] && !wcscmp(g_currCategory, L"~未分类"))))
    {
        itemCnt = ListView_GetItemCount(g_hWndNet);
...
static void ChangePage(HWND hWndParent, int newPage, int newSubPage)
{
    HWND hwndOld, hwndNew;

    if (g_currPage == newPage && g_currSubPage == newSubPage)
        return;

    switch(g_currPage)
    {
    case PAGE_DOWNLOADING: hwndOld = g_hWndDownloading; break;
    default: hwndOld = g_hWndNet; break;
    }
    g_currPage = newPage;
    switch(g_currPage)
    {
    case PAGE_DOWNLOADING: hwndNew = g_hWndDownloading; break;
    default: hwndNew = g_hWndNet; break;
    }
    if (hwndOld != hwndNew)
    {
    ...
    g_currSubPage = newSubPage;
    if (g_currPage==PAGE_NET_IDX || g_currPage==PAGE_LOCAL_IDX)
    {
...
static void NetIdx_OnNotify(NMHDR *lpNmhdr)
{
    ...
    NMITEMACTIVATE *lpnmitem;
    HMENU hMenu, hMenuTrackPopup, hMenuDel;
    POINT pt;
    TCHAR tszTmp[256], tszId[64];
    int cnt;
    ...
    case NM_RCLICK:
        ...
        if (hMenu)
        {
            ...
            if (!ptrList_size(selected))
            {
                EnableMenuItem(hMenuTrackPopup, IDM_START_DOWNLAOD, MF_BYCOMMAND|MF_GRAYED);
                EnableMenuItem(hMenuTrackPopup, IDM_START_SEED, MF_BYCOMMAND|MF_GRAYED);
            ...
            if (1!=ptrList_size(selected))
            {
                ...
                EnableMenuItem(hMenuTrackPopup, cnt-3, MF_BYPOSITION|MF_GRAYED);
            ...
            if (!g_uploader)
            {
                DeleteMenu(hMenuTrackPopup, cnt-1, MF_BYPOSITION);
                DeleteMenu(hMenuTrackPopup, cnt-2, MF_BYPOSITION);
                hMenuDel = GetSubMenu(hMenuTrackPopup, cnt-3);
                DeleteMenu(hMenuDel, 1, MF_BYPOSITION);
            }
            TrackPopupMenu(hMenuTrackPopup, TPM_LEFTALIGN|TPM_LEFTBUTTON,
                pt.x, pt.y, 0, g_hWndMain, NULL);
            DestroyMenu(hMenu);
...
INT_PTR CALLBACK DelNetIdxDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        SetDlgItemTextA(hDlg, IDC_PWD, g_serverPwd);
        return (INT_PTR)TRUE;
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            if (strcmp(g_pwd, g_serverPwd)) SaveOptions();
        case IDCANCEL:
...
static void _NetIdx_RemoveNet()
{
    ...
    {
        if (!CmdSocket_DeleteResource(s, (CHAR *)selected->data, g_pwd))
        {
            _stprintf_s(tszTmp, 256, _T("删除网络资源:%, 操作失败!"), MbcsToTSTR((CHAR *)selected->data, tszId, 32));
            MessageBox(g_hWndMain, tszTmp, g_szAppName, MB_OK|MB_ICONINFORMATION);
        }
        CmdSocket_Close(s);
    }
...
int g_searchEditSelected = 0;
const TCHAR g_searchPrompt[] = _T("键入\"ms\"搜索\"魔兽\"...");
BOOL g_searchLocal = FALSE;
DWORD g_lastSearchKeyTime = 0;
WCHAR g_searchString[64] = { 0 };
HANDLE g_hEventSearchStopped = NULL;
HANDLE g_hEventSearch = NULL;
BOOL g_stopSearchThread = FALSE;
int g_pageBeforeSearch = PAGE_NET_IDX;
int g_subPageBeforeSearch = 0;

extern WCHAR *GetSpellString(WCHAR *_in, WCHAR *_out, int _outLen);

static unsigned __stdcall SearchThreadProc(LPVOID param)
{
    struct idx_net *idxn;
    struct idx_local *idxl, idxlf = { 0 };
    WCHAR wszName[MAX_NAME_LEN], wszPY[MAX_NAME_LEN];
    LVITEM lvI = { 0 };
    TCHAR tszText[512];
    int i, j, itemCount;
    ...
    while (1)
    {
        if (WaitForSingleObject(g_hEventSearch, INFINITE)!=WAIT_OBJECT_0) break;
        if (g_stopSearchThread) break;

        ResetEvent(g_hEventSearch);
        ptrArray_free(&g_searchRes, NULL);
        if (g_currPage!=PAGE_SEARCH) continue;

        _wcslwr_s(g_searchString, 64);
        ...
        for (i=0; i<ptrArray_size(&g_netIdx); i++)
        {
            ...
            wcscpy_s(wszName, MAX_NAME_LEN, idxn->name);
            _wcslwr_s(wszName, MAX_NAME_LEN);
            if (wcsstr(wszName, g_searchString))
                ptrArray_insertSorted(&g_searchRes, idxn);
            else
            {
                GetSpellString(wszName, wszPY, MAX_NAME_LEN);
                if (wcsstr(wszPY, g_searchString))
                    ptrArray_insertSorted(&g_searchRes, idxn);
            }
        }
        ...
        for (i=0; i<ptrArray_size(&g_searchRes); i++)
        {
            idxn = (struct idx_net *)ptrArray_nth(&g_searchRes, i);
            lvI.iItem = i;
            lvI.iImage = ICON_NOTHING;
            ...
            if (WaitForSingleObject(g_hEventSearch, 0)==WAIT_OBJECT_0)
                break;
        }
        for (j=itemCount; j>i; j--)
            ListView_DeleteItem(g_hWndNet, j-1);
    }

    SetEvent(g_hEventSearchStopped);

    return 0;
}

static BOOL BeginSearchThread()
{
    g_hEventSearchStopped = CreateEvent(NULL,TRUE, FALSE, NULL);
    g_hEventSearch = CreateEvent(NULL,TRUE, FALSE, NULL);
    CloseHandle((HANDLE)_beginthreadex(NULL, 0, SearchThreadProc, NULL, 0, NULL));
...
{
    if (!g_hEventSearchStopped) return;

    g_stopSearchThread = TRUE;
    SetEvent(g_hEventSearch);

    while(TRUE)
    {
        if (WAIT_OBJECT_0==MsgWaitForMultipleObjects(1, &g_hEventSearchStopped, FALSE, INFINITE, QS_ALLINPUT))
            break;
        else
        {
            MSG msg;
    ...
    CloseHandle(g_hEventSearchStopped);
    CloseHandle(g_hEventSearch);
    g_hEventSearchStopped = NULL;
    g_hEventSearch = NULL;
}
...
LRESULT CALLBACK SearchEditProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_LBUTTONDOWN:
        lRes = CallWindowProc(g_oldEditProc, hWnd, message, wParam, lParam);
    ...
    case WM_CHAR:
        lRes = CallWindowProc(g_oldEditProc, hWnd, message, wParam, lParam);
        if (wParam == 0x1B)
            PostMessage(GetParent(hWnd), WM_COMMAND, MAKELONG(GetDlgCtrlID(hWnd), 1), (LPARAM)hWnd);
        return lRes;
    }
...
static void ToolBar_Create(HWND hWnd)
{
    ...
    TBBUTTON tbButtons[NUM_BUTTONS] =
    {
        ...
        { MAKELONG(2, 0), IDM_WORKING, TBSTATE_ENABLED,
            BTNS_AUTOSIZE|TBSTYLE_CHECKGROUP, {0}, 0, (INT_PTR)_T("正在下载") },
        { MAKELONG(3, 0), IDM_NET, TBSTATE_ENABLED,
            BTNS_AUTOSIZE|TBSTYLE_CHECKGROUP, {0}, 0, (INT_PTR)_T("网络资源") },
        { MAKELONG(4, 0), IDM_LOCAL, TBSTATE_ENABLED,
            BTNS_AUTOSIZE|TBSTYLE_CHECKGROUP, {0}, 0, (INT_PTR)_T("本地资源") },
        { 0, 0, TBSTATE_ENABLED, BTNS_SEP, {0}, 0, 0},
        { MAKELONG(5, 0), IDM_OPTIONS, TBSTATE_ENABLED, 
            BTNS_AUTOSIZE, {0}, 0, (INT_PTR)_T("系统选项") },
        { MAKELONG(6, 0), IDM_ABOUT, TBSTATE_ENABLED, 
            BTNS_AUTOSIZE, {0}, 0, (INT_PTR)_T("帮助信息") },
        { 0, 0, TBSTATE_ENABLED, BTNS_SEP, {0}, 0, 0 },
        { MAKELONG(7, 0), IDM_EXIT, TBSTATE_ENABLED, 
    ...
    hImageList = ImageList_Create(48, 48, ILC_COLOR32|ILC_MASK, 8, 0);
    ...
    ImageList_AddIcon(hImageList, hiconItem); DestroyIcon(hiconItem);
    hiconItem = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_NET_));
    ImageList_AddIcon(hImageList, hiconItem); DestroyIcon(hiconItem);
    hiconItem = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_LOCAL_));
    ImageList_AddIcon(hImageList, hiconItem); DestroyIcon(hiconItem);
    ...
    if (!g_uploader)
    {
        SendMessage(g_hWndToolBar, TB_DELETEBUTTON, 2, 0);
        SendMessage(g_hWndToolBar, TB_DELETEBUTTON, 1, 0);
    ...
    SetRect(&rcEdit, 0, 0, 10, 12); MapDialogRect(g_hWndTmpl, &rcEdit);
    g_hWndSearch = CreateWindowEx(0, _T("Edit"), g_searchPrompt,
        WS_CHILD|WS_BORDER|WS_VISIBLE|ES_LEFT,
        rc.right+20, (rc.top+rc.bottom-rcEdit.bottom)/2, 190, rcEdit.bottom,
        hWnd, (HMENU)ID_SEARCH, g_hInstance, 0);
    SetParent(g_hWndSearch, g_hWndToolBar);
    SendMessage(g_hWndSearch, WM_SETFONT,
        SendDlgItemMessage(g_hWndTmpl, IDC_TAB, WM_GETFONT, 0, 0), 0);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        // 初始页面
        ChangePage(hWnd, PAGE_DOWNLOADING, 0);
        SendMessage(g_hWndToolBar, TB_SETSTATE, IDM_WORKING, MAKELONG(TBSTATE_CHECKED|TBSTATE_ENABLED, 0));
        // 定时器
    ...
    case WM_TIMER:
        if (wcslen(g_searchString)>=2 &&
            g_lastSearchKeyTime &&
            GetTickCount() >= g_lastSearchKeyTime+500)
        {
            SetEvent(g_hEventSearch);
            g_lastSearchKeyTime = 0;
        }
        break;
    ...
    case WM_COMMAND:
        ...
        case IDM_WORKING:
            ...
            break;
        case IDM_NET:
            ChangePage(hWnd, PAGE_NET_IDX, 0);
            TreeView_SelectItem(g_hWndTree, NULL);
            break;
        case IDM_LOCAL:
            ChangePage(hWnd, PAGE_LOCAL_IDX, 0);
            TreeView_SelectItem(g_hWndTree, NULL);
            break;
        ...
        case ID_SEARCH:
            if (wmEvent==1) // escape
            {
                SetWindowText(g_hWndSearch, _T(""));
                SetFocus(g_hWndNet);
            }
            else if (wmEvent==EN_KILLFOCUS)
            {
                GetWindowTextW((HWND)lParam, g_searchString, 20);
                if (!wcslen(g_searchString))
                {
                    SetWindowText((HWND)lParam, g_searchPrompt);
                    g_searchEditSelected = 0;

                    ChangePage(hWnd, g_pageBeforeSearch, g_subPageBeforeSearch);
                    switch (g_currPage)
                    {
                    case PAGE_DOWNLOADING: iButton = IDM_WORKING; break;
                    case PAGE_NET_IDX: iButton = IDM_NET; break;
                    default: iButton = IDM_LOCAL; break;
                    }
                    SendMessage(g_hWndToolBar, TB_SETSTATE, iButton, MAKELONG(TBSTATE_ENABLED|TBSTATE_CHECKED, 0));
                }
            }
            else if (wmEvent==EN_CHANGE)
            {
                ...
                if (wcslen(g_searchString) >= 2)
                {
                    g_lastSearchKeyTime = GetTickCount();
                    if (g_currPage != PAGE_SEARCH)
                    {
                        g_searchLocal = (g_currPage==PAGE_LOCAL_IDX);
                        SendMessage(g_hWndToolBar, TB_SETSTATE, IDM_WORKING, MAKELONG(TBSTATE_ENABLED, 0));
                        SendMessage(g_hWndToolBar, TB_SETSTATE, IDM_NET, MAKELONG(TBSTATE_ENABLED, 0));
                        SendMessage(g_hWndToolBar, TB_SETSTATE, IDM_LOCAL, MAKELONG(TBSTATE_ENABLED, 0));
                        g_pageBeforeSearch = g_currPage;
                        g_subPageBeforeSearch = g_currSubPage;
                        ChangePage(hWnd, PAGE_SEARCH, 0);
                        TreeView_SelectItem(g_hWndTree, NULL);
                    }
                }
                else if (g_currPage == PAGE_SEARCH)
                    ListView_DeleteAllItems(g_hWndNet);
            }
            break;

        case IDM_CHANGE_PAGE:
            ChangePage(hWnd, LOWORD(lParam), HIWORD(lParam));
            break;
...
INT_PTR CALLBACK UploadNewDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    CHAR id[MAX_ID_LEN], pwd[MAX_PWD_LEN], serversNotify[1024];
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            GetDlgItemTextA(hDlg, IDC_PEERS, serversNotify, 1024);
            SaveOptions();
            s = CmdSocket_CreateAndConnect(g_serverIp, g_serverPort);
            if (s != INVALID_SOCKET)
            {
                if (!CmdSocket_UploadResource(s, id, dir, cate, pwd, serversNotify))
                {
                ...
                if (strcmp(pwd, g_serverPwd) || strcmp(serversNotify, g_serversNotify))
                    SaveOptions();
            }
            else
            {
...
INT_PTR CALLBACK UploadBatchDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDC_BROWSE:
            ...
            if (pidl)
            {
                ...
                for (i=0,li=subDirs; li; i++,li=li->next)
                {
                    ...
                    UnicodeToTSTR((WCHAR *)li->data, szPath, MAX_PATH);
        ...
        case IDOK:
            ...
            GetDlgItemTextW(hDlg, IDC_CATEGORY, cate, MAX_CATEGORY_LEN);
            GetDlgItemTextA(hDlg, IDC_PWD, pwd, MAX_PWD_LEN);
            GetDlgItemTextA(hDlg, IDC_PEERS, serversNotify, 1024); SaveOptions();
            s = CmdSocket_CreateAndConnect(g_serverIp, g_serverPort);
            if (s != INVALID_SOCKET)
            {
                iId = atoi(id);
...
static void PeerInfo_FillListView(HWND hDlg, struct ptrList *peers)
{
    ...
    int currCount, i, j;
    LV_ITEM lvI = { 0 };
    TCHAR tszId[MAX_ID_LEN], szText[256], szTmp[256];
    ...
    for (li=peers, i=0; li; li=li->next, i++)
    {
        ...
        lvI.mask = LVIF_TEXT;
        lvI.iItem = i;
        lvI.pszText = MbcsToTSTR(pr->pid, tszId, MAX_ID_LEN);
...
INT_PTR CALLBACK PeerInfoDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        if (idxn)
        {
            ...
            lvI.mask = LVIF_TEXT;
            lvI.iItem = 0; lvI.pszText = _T("ID/Hash");
